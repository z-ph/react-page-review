...
### Requirement: 默认 UI 使用 Ant Design
WHEN 用户安装并使用 `ReviewTool` 组件
THEN 系统 SHALL 通过 antd 渲染工具栏、弹窗、抽屉和表单
AND antd SHALL 作为可选 peer dependency 提供

#### Scenario: 安装依赖
GIVEN 用户项目中已安装 React 18
WHEN 用户安装 `react-page-review`
THEN 用户 SHALL 自行安装 `antd`
AND `react-page-review` 不应强制安装 antd
