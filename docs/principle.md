# react-page-review 原理与实现

本文档从源码层面说明 `react-page-review` 的架构、核心功能实现细节以及安全与性能方面的考虑。
...
| 层级 | 文件 | 职责 |
| --- | --- | --- |
| UI 层 | `src/ReviewTool.jsx` | 工具栏、元素高亮、框选框、表单弹窗、抽屉、评审列表、确认弹窗 |
...
公共入口 `src/index.jsx` 只导出 `ReviewTool` 与 `usePageReview`，截图与检查模块为内部实现。
...
点击有效目标时，会调用 `e.preventDefault()` 与 `e.stopPropagation()`，并生成包含 `el`、`selector`、`tag`、`text`、`rect`、`docRect` 的选择项。
...
`onScroll` 会更新 `scrollPos`，并调用 `setSelectedElements` 重新查询元素并更新 `rect`，使高亮框始终跟随目标。
...
React 版只支持自定义 `imageUpload` 函数，没有内置 URL 上传器。
...
每次 `addReview`、`updateReview`、`deleteReview`、`clearPageReviews` 后都会调用 `saveToStorage`，将当前数组完整序列化写入。React 版使用函数式 `setReviews` 更新状态，并在回调中写入 localStorage，确保状态与缓存一致。写入失败会被静默吞掉，避免阻塞 UI。
...
React 版使用 `useRef` 保存拖拽过程中的中间状态（如起始坐标、原始尺寸），避免每次鼠标移动都触发重渲染。UI 状态如 `toolbarPos`、`modalSize`、`selectedBoxes` 等使用 `useState`。所有事件处理器通过 `useCallback` 缓存，并在 `useEffect` 的依赖数组中正确声明，保证事件在 `active` 变化时正确绑定或清理。
...
2. **上传安全**：自定义 `imageUpload` 会将截图上传到服务端，请确保接口有认证与文件类型校验。
...
4. **内存泄漏**：组件卸载时 `useEffect` 的 cleanup 函数会移除所有监听；下载生成的 Blob URL 在 1 秒后清理，兼顾自动化工具捕获需求。
5. **React 渲染优化**：拖拽与缩放过程中大量中间状态使用 `useRef`，只有真正需要展示的状态才通过 `setState` 更新。
